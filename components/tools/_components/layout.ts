...
export const DEFAULT_PAPER_STOCK: PaperStockSettings = {
  ...
  topMargin: 0.507, leftMargin: 0.75, horizontalGap: 0, verticalGap: 0, bleed: 0.125, showGuides: true,
  columns: 2, rows: 5,
  cornerOffsets: {
...
export const drawPerforatedCornerGuide = (
  ...
  const { x: guideX, y: guideY } = getPerforatedCornerGuideCenter(layout, guideIndex);
  ...
  context.setLineDash([]);
  context.beginPath();
  context.moveTo(guideX - length / 2, guideY);
  context.lineTo(guideX + length / 2, guideY);
  context.moveTo(guideX, guideY - length / 2);
  context.lineTo(guideX, guideY + length / 2);
  context.stroke();
};
